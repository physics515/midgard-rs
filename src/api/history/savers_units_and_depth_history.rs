...
use crate::{APIError, Interval, SaversHistory};
...
pub async fn api_get_savers_units_and_depth_history(base_url: &str, pool: &str, interval: Option<Interval>, count: Option<usize>, to: Option<u64>, from: Option<u64>) -> Result<SaversHistory, APIError> {
	let mut endpoint = base_url.to_string() + "history/savers/" + pool;
	...
	let res: SaversHistory = serde_json::from_str(&response)?;
